...
**Last updated:** July 12, 2026 (Session 53 — doc-update automation + ASVS audit skill, incl. 53.3 dry run)
...
**Doc-update hook is dry-run only, pending Daniel's go-ahead.** `.husky/post-commit` → `scripts/check-docs-updated.sh` fires on every `git commit` (Daniel's or Claude's) and deterministically flags commits that touch substantive paths (`src/**`, `supabase/migrations/**`, `docs/design-and-specs/**`, the ASVS docs) without a paired STATUS.md/session-note update — but it currently only logs to `.git/hook-logs/check-docs-updated.log`, it doesn't launch the headless `doc-updater` run. That's gated behind a local, untracked flag file (`.claude/doc-hook-enabled`, in `.gitignore`) that doesn't exist yet. Deliberate scope-back: unattended commit automation with its own `git add`/`git commit` permissions was judged a bigger decision than the rest of session 53's edits. Open question is simply timing — how long to watch the dry-run log before creating the flag file. See Next Steps.
...
**Tooling decision pending:** Doc-update hook (`scripts/check-docs-updated.sh`, session 53) is live but dry-run only — decide whether to create `.claude/doc-hook-enabled` after watching `.git/hook-logs/check-docs-updated.log` for a while.

**Recently completed:**
- Session 53 — Workflow/tooling session, no build spec. Added a native git post-commit hook (via Husky, not a Claude Code hook — deliberate, since a Claude-Code-specific hook would miss commits Daniel makes directly) that deterministically flags commits touching substantive paths without a paired doc update; currently dry-run only (logs to `.git/hook-logs/check-docs-updated.log`), gated behind the untracked `.claude/doc-hook-enabled` flag pending Daniel's go-ahead. Fixed a `git diff-tree` bug requiring `--root` for a repo's first commit (found via scratch-repo testing). `doc-updater-agent.md` taught its headless entry point (`git show <sha>` instead of a pasted diff, when invoked automatically). Also added `.claude/skills/asvs-audit/SKILL.md` — a guided per-chapter workflow for the ASVS L1 checklist (scaffolded earlier the same day, `3e2906a`), built around three explicit understanding checkpoints rather than an automated fill-in-the-table pass. Later the same day (53.3), ran the skill's planned dry run, `/asvs-audit V13` — verified `V13.4.1` (no exposed `.git`/`.svn` metadata) via direct `curl` against production, confirming Vercel's SPA rewrite returns `index.html` for `/.git/*` rather than real git data. V13 (Configuration) is now fully resolved; 1 of 70 L1 requirements verified overall. Also served as a live confirmation that the doc-update hook's dry-run logging works correctly against a real commit (`c04c528`).
