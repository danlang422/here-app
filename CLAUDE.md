...
- `20260714171604` — Revoked `anon`'s execute grant on `get_my_organization_id()` and `activity_is_visible_to_all()`, the two SECURITY DEFINER helpers missed when the other three were locked to `authenticated` in `20260513132144`. Found during the ASVS V8 audit via live grant inspection; no data exposure (both key off `auth.uid()`, `NULL` for `anon`), grant-hygiene fix only.
- `20260714173806` — Adds `prevent_privileged_self_edit()`, a BEFORE UPDATE trigger on `user_profiles` blocking changes to `roles`/`organization_id`/`is_active` on a user's own row unless the acting user already holds `admin`. Fixes a real privilege-escalation gap found during the ASVS V8 audit: the own-row UPDATE RLS policy (`id = auth.uid()`) had no column restriction, so any authenticated user could self-assign these fields via a direct REST call (not reachable through the app UI, but reachable via any valid session token). Verified live against real rows via rolled-back transactions.
- `20260714175823` — Fixes `activity_terms_staff_select` RLS policy, which checked for a `'staff'` role value never actually assigned in `user_profiles.roles` (real values: `student`/`teacher`/`admin`) — a typo for `'teacher'` from the original migration. Dead code, fail-closed, no security exposure; flagged in session 54, fixed during the ASVS V8 audit (session 55).
